...
`module  JAC.GreenFunction`  
    ... a submodel of JAC that contains all methods for computing an approximate many-electron Green function.
"""
module GreenFunction
    ...
    """
    `struct  GreenFunction.Outcome`  
        ... defines a type to keep the outcome of a Green function approximation.
    ...
    """
    `GreenFunction.Outcome()`  ... constructor for an `empty` instance of GreenFunction.
    """
    function Outcome()
        Outcome(Level(), 0.)
    end
    ...
    """
    `GreenFunction.computeOutcomes(multiplet::Multiplet, nm::Nuclear.Model, grid::Radial.Grid, settings::GreenFunction.Settings; output=true)` 
        ... to compute (as selected) the alpha-variation parameters for the levels of the given multiplet and as 
            specified by the given settings. The results are printed in neat tables to screen but nothing is returned 
            otherwise.
    """
    function computeOutcomes(multiplet::Multiplet, nm::Nuclear.Model, grid::Radial.Grid, settings::GreenFunction.Settings; output=true)
        println("")
        printstyled("GreenFunction.computeOutcomes(): The computation of approximate Green functions starts now ... \n", color=:light_green)
        printstyled("---------------------------------------------------------------------------------------------- \n", color=:light_green)
        #
    ...
    """
    `GreenFunction.displayOutcomes(outcomes::Array{GreenFunction.Outcome,1})`  
        ... to display a list of levels that have been selected for the computations. A small neat table of all selected levels 
            and their energies is printed but nothing is returned otherwise.
    """
    ...
    """
    `GreenFunction.displayResults(stream::IO, outcomes::Array{GreenFunction.Outcome,1})`  
