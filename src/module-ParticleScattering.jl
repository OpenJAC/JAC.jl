...
module ParticleScattering
    ...
    function amplitude(processType::ElasticElectronNR, beamType::Beam.BesselBeam, 
        ...
        if  l >= abs(beamType.mOAM)
            ...
            wa        = GSL.sf_legendre_Plm(l, abs(beamType.mOAM), cos(beamType.openingAngle))
            if   beamType.mOAM < 0    wa = (-1)^beamType.mOAM * wa / prod(i for i = l-beamType.mOAM+1.0:l+beamType.mOAM)   end
            wb        = 4pi * (-1.0im)^beamType.mOAM / k * exp( im*lPhase ) * (-1.)^beamType.mOAM                  *
                        ...
                        wa * AngularMomentum.sphericalYlm(l, beamType.mOAM, theta, phi) * sin(lPhase)
    ...
    function computeAmplitudesProperties(processType::ElasticElectronNR, event::ParticleScattering.EventNR, nm::Nuclear.Model, 
        ...
        for  l = 0:1000
            ...
            amp1 = amp2;   amp2 = abs(amplitude)^2 
            if  amp1 + amp2 < settings.epsPartialWave    break     end
        end
