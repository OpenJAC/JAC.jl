...
"""
`Bsplines.generateMatrix!(kappa::Int64, TTp::String, primitives::Bsplines.Primitives, nsL::Int64, nsS::Int64, storage::Dict{Array{Any,1},Array{Float64,2}})`  
...
"""
function generateMatrix!(kappa::Int64, TTp::String, primitives::Bsplines.Primitives, nsL::Int64, nsS::Int64, storage::Dict{Array{Any,1},Array{Float64,2}})
...
function generateOrbitalFromPrimitives(energy::Float64, sh::Subshell, mtp::Int64, ev::Array{Float64,1}, primitives::Bsplines.Primitives)
    nsL = primitives.grid.nsL - 1;    nsS = primitives.grid.nsS - 1
    P = zeros(primitives.grid.NoPoints);    Pprime = zeros(primitives.grid.NoPoints)
    Q = zeros(primitives.grid.NoPoints);    Qprime = zeros(primitives.grid.NoPoints)
    for  i = 1:nsL   
        for  j = primitives.bsplinesL[i].lower:primitives.bsplinesL[i].upper  P[j]      = P[j] + ev[i] * primitives.bsplinesL[i].bs[j]      end
    ...
    #== Determine the maximum number of grid points for this orbital and normalized it propery
    mtp = 0;   for j = primitives.grid.NoPoints:-1:1    if  abs(P[j]) + abs(Q[j]) > 2.0e-10   mtp = j;   break   end     end
    Px = zeros(mtp);    Qx = zeros(mtp);    Px[1:mtp] = P[1:mtp];    Qx[1:mtp] = Q[1:mtp]    
    Pprimex = zeros(mtp);    Qprimex = zeros(mtp);    Pprimex[1:mtp] = Pprime[1:mtp];    Qprimex[1:mtp] = Qprime[1:mtp]    
    for  j = 1:mtp   if  abs(Px[j]) < 1.0e-10    Px[j] = 0.   end
                        if  abs(Qx[j]) < 1.0e-10    Qx[j] = 0.   end 
...
function generatePrimitives(grid::Radial.Grid)
    ...
    #
    for  i = 2:grid.nsL
        bspline = zeros( grid.NoPoints );   bprime = zeros( grid.NoPoints )
        # Suppress a finite value at r = 0 and at rmax by  j = 2:...
        for  j = 1:grid.NoPoints     bspline[j] = Bsplines.generateSpline(i, grid.orderL, grid.tL, grid.r[j])                  end
        if     bspline[1] > 0.   lower = 1
        else   for  j = 2:grid.NoPoints       if  bspline[j] > 0.   lower = j-1;   break   end  end    
        end
        for  j = grid.NoPoints-1:-1:1    
        ...
        #== if  i == grid.nsL   
            plt.figure()  
            plt.plot(grid.r[1:grid.NoPoints], bspline[1:grid.NoPoints])
            plt.plot(grid.r[1:grid.NoPoints], bprime[1:grid.NoPoints])  
            plt.show()
        end ==#
        #
    ...
    #
    for  i = 2:grid.nsS
        bspline = zeros( grid.NoPoints );   bprime = zeros( grid.NoPoints );   lower = 0;   upper = 0
        # Suppress a finite value at r = 0 at rmax  by  j = 2:...
        for  j = 1:grid.NoPoints     bspline[j] = Bsplines.generateSpline(i, grid.orderS, grid.tS, grid.r[j])                  end
        if     bspline[1] > 0.   lower = 1
        else   for  j = 2:grid.NoPoints       if  bspline[j] > 0.   lower = j-1;   break   end  end    
        end
        for  j = grid.NoPoints-1:-1:1    
...
"""
`Bsplines.generateGalerkinMatrix(primitives::Bsplines.Primitives, nsL::Int64, nsS::Int64, energy::Float64, sh::Subshell, pot::Radial.Potential)`  
...
function generateGalerkinMatrix(primitives::Bsplines.Primitives, nsL::Int64, nsS::Int64, energy::Float64, sh::Subshell, pot::Radial.Potential)
    ...
    wa = setupLocalMatrix(sh.kappa, primitives, nsL, nsS, pot::Radial.Potential, storage::Dict{Array{Any,1},Array{Float64,2}})
...
function generateOrbitalsForPotential(primitives::Bsplines.Primitives, kappa::Int64, subshells::Array{Subshell,1}, 
    ...
    storage  = Dict{Array{Any,1},Array{Float64,2}}()
    # Set-up the overlap matrix
    nsL = primitives.grid.nsL - 2;    nsS = primitives.grid.nsS - 2
    wb = zeros( nsL+nsS, nsL+nsS )
    ...
    nsi = nsS;    if kappa > 0   nsi = nsi + 1 - 1   end
    ...
    for  sh in subshells
        orbitals = Base.merge( orbitals, Dict( sh => generateOrbitalFromPrimitives(sh, w2, primitives, nsL, nsS) ))
    end
...
function generateOrbitalsHydrogenic(primitives::Bsplines.Primitives, nuclearModel::Nuclear.Model, subshells::Array{Subshell,1}; 
    ...
    for  i = 1:length(subshells)
        ...
        wa = Bsplines.setupLocalMatrix(sh.kappa, primitives, nsL, nsS, pot, storage)
        w2 = Basics.diagonalize("generalized eigenvalues: LinearAlgebra", wa, wb)
        ##x wa = LinearAlgebra.Symmetric(wa)
        ...
        for  j = 1:length(subshells)
            ...
            elseif  sh.kappa == subshells[j].kappa   
                newOrbitals[subshells[j]] = generateOrbitalFromPrimitives(subshells[j], w2, primitives, nsL, nsS)
...
function  solveSelfConsistentALField(primitives::Bsplines.Primitives, 
    ...
    while  true
        ...
        for (ish, sh) in  enumerate(basis.subshells)
            ...
            wa = Bsplines.setupLocalMatrix(sh.kappa, primitives, nsL, nsS, nuclearPotential, storage)
            wa = wa + wde
...
function solveSelfConsistentMeanField(primitives::Bsplines.Primitives, nuclearModel::Nuclear.Model, basis::Basis, 
    ...
    while  isNotSCF
        ...
        for kappa in kappas
            ...
            wp  = Basics.computePotential(settings.scField, grid, wLevel)
