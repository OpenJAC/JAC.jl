...
function give(sa::String)
    ...
    elseif    sa == "framework"                                         return (JAC_FRAMEWORK)
    elseif    sa == "method: continuum"                                 return (JAC_CONTINUUM)
    elseif    sa in ["electron rest energy", "mc^2"]                    return (1/(FINE_STRUCTURE_CONSTANT^2))   
    elseif    sa == "unit: energy"                                      return (JAC_ENERGY_UNIT)
    elseif    sa == "unit: cross section"                               return (JAC_CROSS_SECTION_UNIT)
    elseif    sa == "unit: rate"                                        return (JAC_RATE_UNIT)
    elseif    sa == "unit: time"                                        return (JAC_TIME_UNIT)
    elseif    sa == "standard grid"                                     return (JAC_STANDARD_GRID)
    elseif    sa == "speed of light: c"                                 return (1.0/FINE_STRUCTURE_CONSTANT)
    elseif    sa == "summary flag/stream"                               return ( (JAC_PRINT_SUMMARY, JAC_SUMMARY_IOSTREAM) )
    elseif    sa == "test flag/stream"                                  return ( (JAC_PRINT_TEST,    JAC_TEST_IOSTREAM) )
