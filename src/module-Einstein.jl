...
"""
`module  JAC.Einstein`  ... a submodel of JAC that contains all methods for computing Einstein A and B coefficients, treated as level 
                            properties with regard to the levels of a common multiplet; it is using JAC, JAC.ManyElectron, JAC.Radial.
"""
module Einstein
    ...
    """
    `struct  Settings`  ... defines a type for the details and parameters of computing Einstein lines and coefficients
    ...
    """
    `JAC.Einstein.Settings()`  ... constructor for the default values of Einstein line computations.
    """
    function Settings()
    ...
    """
    `JAC.Einstein.Settings(multipoles::Array{EmMultipole,1};` printBeforeComputation::Bool=false, 
    ...
    """
    `struct  Channel`  ... defines a type for a single Einstein emission/absorption channel that specifies the multipole, gauge and amplitude.
    ...
    """
    `struct  Line`  ... defines a type for a Einstein line that may include the definition of sublines and their corresponding amplitudes.
        ...
        + hasSublines    ::Bool             ... Determines whether the individual sublines are defined in terms of their multipolarity, 
                                                amplitude, or not.
    ...
    """
    `JAC.Einstein.Line(initialLevel::Level, finalLevel::Level, photonRate::Float64)`  ... constructor an Einstein line between a
    ...
    """
    `JAC.Einstein.computeLines(multiplet::Multiplet, grid::Radial.Grid, settings::Einstein.Settings; output::Bool=true)`  ... to compute the 
         Einstein transition amplitudes and all properties as requested by the given settings. A list of lines::Array{Einstein.Lines} is returned.
    """
    function  computeLines(multiplet::Multiplet, grid::Radial.Grid, settings::Einstein.Settings; output::Bool=true)
        println("")
        printstyled("JAC.Einstein.computeLines(): The computation of the transition amplitudes and properties starts now ... \n", color=:light_green)
        printstyled("------------------------------------------------------------------------------------------------------- \n", color=:light_green)
        println("")
        lines = JAC.Einstein.determineLines(multiplet, settings)
        # Display all selected lines before the computations start
        if  settings.printBeforeComputation    JAC.Einstein.displayLines(lines)    end
        # Calculate all amplitudes and requested properties
        newLines = Einstein.Line[]
        for  line in lines
            newLine = JAC.Einstein.computeAmplitudesProperties(line, grid, settings) 
    ...
    """
    `JAC.Einstein.computeAmplitudesProperties(line::Einstein.Line, grid::Radial.Grid, settings::Einstein.Settings)`  ... to compute all 
    ...
    """
    `JAC.Einstein.computeMatrix_obsolete(Mp::EmMultipole, gauge::EmGauge, omega::Float64, finalLevel::Level, initialLevel::Level, grid::Radial.Grid, 
                                settings::Einstein.Settings)`  ... to compute the matrix Mp = (<csf_r|| O^Mp (gauge) ||csf_s>) of the multipole 
         interaction for the given CSF basis and transition energy ``ω``. A (length(basis.csfs) x length(basis.csfs)}-dimensional 
    ...
    function  computeMatrix_obsolete(Mp::EmMultipole, gauge::EmGauge, omega::Float64, finalLevel::Level, initialLevel::Level, grid::Radial.Grid, 
        ...
        for  r = 1:n
            ...
            for  s = 1:n
                ...
                me = 0.
                for  coeff in wa
                    me = me + coeff.T * JAC.InteractionStrength.MbaCheng(Mp, gauge, omega, finalLevel.basis.orbitals[coeff.a],  
    ...
    """
    `JAC.Einstein.determineChannels(finalLevel::Level, initialLevel::Level, settings::Einstein.Settings)`  ... to determine a list of 
         Einstein.Channel for a transitions from the initial to final level and by taking into account the particular settings of for this
         computation; an Array{Einstein.Channel,1} is returned.
    """
    function  determineChannels(finalLevel::Level, initialLevel::Level, settings::Einstein.Settings)
        ...
        for  mp in settings.multipoles
            if   JAC.AngularMomentum.isAllowedMultipole(symi, mp, symf)
                if     string(mp)[1] == 'E'   push!(channels, Einstein.Channel(mp, JAC.Coulomb,   0.) )
                                              push!(channels, Einstein.Channel(mp, JAC.Babushkin, 0.) )  
                elseif string(mp)[1] == 'M'   push!(channels, Einstein.Channel(mp, JAC.Magnetic,  0.) ) 
                else   error("stop a")
                end 
            end
        end
    ...
    """
    `JAC.Einstein.determineLines(multiplet::Multiplet, settings::Einstein.Settings)`  ... to determine a list of EinsteinLine's for transitions 
         between the lines from the given multiplet and by taking into account the particular selections and settings of for this computation; an 
         Array{Einstein.Line,1} is returned. Apart from the level specification, all physical properties are set to zero during the initialization 
         process.
    """
    function  determineLines(multiplet::Multiplet, settings::Einstein.Settings)
        if    settings.selectLines    selectLines   = true
            selectedLines = JAC.determineSelectedLines(settings.selectedLines, multiplet, multiplet)
        ...
        for  i = 1:length(multiplet.levels)
            for  f = 1:length(multiplet.levels)
                ...
                ##x JAC.warn(AddWarning, "determineLines:: omega = $omega  min-omega = $(settings.mimimumPhotonEnergy) ")
    ...
    """
    `JAC.Einstein.displayLifetimes(stream::IO, lines::Array{Einstein.Line,1})`  ... to list all lifetimes of the selected lines as can 
         be derived from the calculated data. A neat table is printed but nothing is returned otherwise.
    """
    ...
    """
    `JAC.Einstein.displayLines(lines::Array{Einstein.Line,1})`  ... to display a list of lines and channels that have been selected due to
    ...
    function  displayLines(lines::Array{Einstein.Line,1})
        ...
        println(" ")
        println("  ", JAC.TableStrings.hLine(115))
        sa = "  ";   sb = "  "
        ...
        println(sa);    println(sb);    println("  ", JAC.TableStrings.hLine(115)) 
        #   
        for  line in lines
    ...
    """
    `JAC.Einstein.displayRates(stream::IO, lines::Array{Einstein.Line,1})`  ... to list all results, energies, rates, etc. of the selected lines.
    ...
    function  displayRates(stream::IO, lines::Array{Einstein.Line,1})
        ...
        sa = sa * JAC.TableStrings.center(14, "Energy"   ; na=4);               
        sb = sb * JAC.TableStrings.center(14,JAC.TableStrings.inUnits("energy"); na=4)
        sa = sa * JAC.TableStrings.center( 9, "Multipole"; na=1);                         sb = sb * JAC.TableStrings.hBlank(10)
        sa = sa * JAC.TableStrings.center(11, "Gauge"    ; na=4);                         sb = sb * JAC.TableStrings.hBlank(16)
        sa = sa * JAC.TableStrings.center(30, "A--Einstein--B"; na=2);       
        sb = sb * JAC.TableStrings.center(30, JAC.TableStrings.inUnits("rate")*"           "*JAC.TableStrings.inUnits("rate"); na=2)
        sa = sa * JAC.TableStrings.center(13, "Osc. strength"    ; na=4);                 sb = sb * JAC.TableStrings.hBlank(20)
        sa = sa * JAC.TableStrings.center(14, "Decay widths"; na=4);       
        sb = sb * JAC.TableStrings.center(14, JAC.TableStrings.inUnits("energy"); na=4)
