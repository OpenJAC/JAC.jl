...
... a submodel of JAC that contains all methods for computing positron-bound-electron pair annihilation (PEPA) with two-photon emission 
    cross sections and rates; e^+ + |i(N)> --> |f(N-1)> + photon + photon. 
"""
module PairAnnihilation2Photon 

using JAC, ..ManyElectron, ..TableStrings


"""
`struct  PairAnnihilation2Photon.Settings`  
    ... defines a type for the details and parameters in computing positron-bound-electron pair annihilation (PEPA) with single-photon 
        emission cross sections and rates; e^+ |i(N)> --> |f(N-1)> + photon1 + photon2.
...
"""
`PairAnnihilation2Photon.Settings()`  ... constructor for the default values of pair-annihilation photon line computations
"""
function Settings()
...
"""
`struct  PairAnnihilation2Photon.Channel`  
    ... defines a type for a positron-bound-electron pair annihilation (PEPA) with two-photon   emission channel that specifies all 
        quantum numbers, phases and amplitudes.
...
"""
`struct  PairAnnihilation2Photon.Line`  
    ... defines a type for a positron-bound-electron pair-annihilation (two-photon) line that may include the definition of channels.
