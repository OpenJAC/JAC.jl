...
#
println("Ed) DR rate coefficients for neon-like Ar: AverageSCA model.")
using JLD
#
if  true
    ...
    grid = Radial.Grid(Radial.Grid(false), rnt = 4.0e-6, h = 5.0e-2, hp = 3.0e-2, rbox = 10.0)
    ...
    wa   = Cascade.Computation(Cascade.Computation(); name=name, nuclearModel=Nuclear.Model(10.), grid=grid, approach=Cascade.AverageSCA(),
                               scheme=Cascade.DrRateCoefficientScheme([ElecCapture(), Auger(), Radiative()], [E1], 3.0, 5.0, 1, 3, 2, 
                                                                      [Shell("2s"), Shell("2p")]),
