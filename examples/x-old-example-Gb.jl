...
elseif true
    ...
    approx      = ImpactIonization.MUIBEDapproximation()
...
elseif false
    ...
    ##x basis       = Basics.performSCF(configs, nucModel, grid, AsfSettings()) 
    ##x eiiSettings = ImpactIonization.Settings(approx, iEnergies, false, true, false, selection)
    ##x display     = ImpactIonization.displayCrossSections(basis, grid, nucModel, iEnergies, shells, eiiSettings, configs)
    comp        = Empirical.Computation(name, nucModel, grid, configs, eiiSettings)
    #
    @show comp
...
elseif false
    ...
    # First suggestion ... likely obsolete now due to Yuan-Cheng's work
