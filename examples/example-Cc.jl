...
wa = Atomic.Computation(Atomic.Computation(), name="xx", grid=JAC.Radial.Grid(true), 
                        nuclearModel=Nuclear.Model(26., "Fermi", 58., 3.81, AngularJ64(5//2), 1.0, 1.0), properties=[Isotope()],
                        # grid=JAC.Radial.Grid(true),
                        configs=[Configuration("[Ne] 3s^2"), Configuration("[Ne] 3s 3p"), Configuration("[Ne] 3p^2")],
                        isotopeSettings=IsotopeShift.Settings(true, true, true, true, false, Int64[], FullCIjulia()) )
...
#==
wa = Atomic.Computation("xx",  Nuclear.Model(26., "Fermi", 58., 3.81, AngularJ64(5//2), 1.0, 1.0); properties=[Isotope()],
